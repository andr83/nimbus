...
class SSJob(config: SSJob.Config) extends Job {
  ...
  override def run(storage: Storage): Cancelable = {
    val alreadyProcessed =
      java.util.Collections.newSetFromMap(new java.util.concurrent.ConcurrentHashMap[Item, java.lang.Boolean]).asScala
      ...
      () => {
        try {
          ...
          val matched = items
            ...
            .filterNot(alreadyProcessed.apply)
            .filter(i => i.price > config.minPrice && i.price < config.maxPrice)
